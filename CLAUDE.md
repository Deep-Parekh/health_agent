...
`docs/PLAN.md` is the single source of truth for project state, and it must stay current:

- **Before starting work**: read `docs/PLAN.md`, especially the "Progress log & notes for
...
- `docs/DEBUG_MODE_PLAN.md` is **parked** until the 4 launch phases finish — don't start it,
  don't delete it.
- `docs/PROGRESS.md` is the committable, user-facing record of completed work — keep it
  current alongside docs/PLAN.md. `docs/architecture.html` is deliberately **git-ignored**
...
- Tests must never construct a bare `UserStore()` — strip `DATABASE_URL` from the env
  first (see E5 in docs/PLAN.md), or they'll silently hit production Postgres.
- Cross-user isolation is load-bearing: ALL profile access goes through
  ...
  (Supabase gotchas E6/E7 in docs/PLAN.md explain why it works the way it does).
...
- Secrets: a real `OPENAI_API_KEY` and `DATABASE_URL` live in local `.env`. **Never read
  ...
  that might embed the connection string. `scripts/verify_deployment.py` is the template.
- Local e2e: `python scripts/verify_deployment.py [--live]` with the OpenAI backend; Ollama
